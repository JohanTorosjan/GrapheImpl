...
public class IncidenceArrayGraph implements Graph{
    ...
    public void addEdge(Vertex vertex, Vertex vertex1,EdgeKind edgeKind) {
        if(nbEdge<this.maxVertex*this.maxVertex){
            this.edgesArray[nbEdge] = new Edge(vertex,vertex);
            this.incidenceArray[vertex.getId()][nbEdge] = this.edgesArray[nbEdge];
    ...
    public boolean isConnected(Vertex vertex, Vertex vertex1) {
        for(int i=0;i<=maxVertex*maxVertex;i++){
            if(incidenceArray[vertex.getId()][i]!=null){
                return incidenceArray[vertex1.getId()][i]!=null;
            }
        }
        return false;
    }
